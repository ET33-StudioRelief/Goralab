...
(() => {
  ...
  function initTagsAppear() {
    const tags = document.querySelectorAll(".tag");
    tags.forEach((tag) => {
      gsapWithCSS.from(tag, {
        scrollTrigger: {
          trigger: tag,
          start: "top 90%",
  ...
  function initAboutTextAppear() {
    const section = document.querySelector(".section_about");
    const content = section?.querySelector(".text-rich-text");
    if (!section || !content) return;
    gsapWithCSS.from(content, {
      scrollTrigger: {
        trigger: section,
        // on déclenche sur toute la section_about
        start: "top 40%",
        toggleActions: "play none none none"
      },
      y: 40,
  ...
  function initEngagementParallax() {
    const section = document.querySelector(".section_engagement");
    if (!section) return;
    const image = section.querySelector(".engagement_img-wrap img");
    if (!image) return;
  ...
  function initIntelligenceEllipses() {
    const circle = document.querySelector('[trigger="ellipse-path"]');
    const orbit = document.querySelector("#ellipse-orbit");
    if (!circle || !orbit) return;
    const cx = parseFloat(circle.getAttribute("cx") ?? "0");
    const cy = parseFloat(circle.getAttribute("cy") ?? "0");
    const r = parseFloat(circle.getAttribute("r") ?? "0");
    if (!r) return;
    const images = orbit.querySelectorAll("image");
